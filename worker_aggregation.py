...
class AggregationWorker:
    def __init__(self, ip_port, clients_amount, model_path="model/aggregator"):
    ...
    def accept_client(self):
        ...
        self.clients.append(AggregationClientStub(connection, address, client_id))
    ...
    def aggregate(self):
        ...
        weights_new = []
        for client in self.clients:
            with open(f"{self.model_path}/client_{client.id}.h5", "wb") as file:
        ...
        for i in range(14):
            ...
            weights_new.append(layer_weights_mean)
        model.set_weights(weights_new)
        model.save(f"{self.model_path}/aggregated.h5")
        with open(f"{self.model_path}/aggregated.h5", 'rb') as file:
...
if __name__ == '__main__':
    ...
    aggregator = AggregationWorker(ip_port=c.BIND_IP_PORT, model_path='model/aggregator', clients_amount=c.CLIENT_NUM)
    aggregator.run()
